...
class bp_course_scheduler{
	...
	function wplms_schedule_user_inactivity_mail(){
	    ...
	    if(!wp_next_scheduled( 'wplms_user_inactivity_mail' )){
    		wp_schedule_single_event(time()+86400,'wplms_user_inactivity_mail');
    	}
	...
	function wplms_user_inactivity_mail(){
	    ...
	    $current_time = time();
	    $time = $current_time - ($inactivity_schedule*86400);
	    $date = date('Y-m-d H:i:s', $time);
		$site_url = home_url();

		$users = $wpdb->get_results("SELECT u.user_email as email FROM {$wpdb->users} as u LEFT JOIN {$wpdb->usermeta} as m ON m.user_id = u.ID AND m.meta_key = 'last_activity' LEFT JOIN {$wpdb->usermeta} as m2 ON  m2.user_id = u.ID AND m2.meta_key = 'inactive' WHERE m.meta_value <= '$date' AND (m2.meta_key IS NULL OR m2.meta_value = 'yes')");
		...
		if(!empty($users)){
	        ...
	        bp_send_email( 'wplms_inactive_user',$to, $bpargs );
		}
